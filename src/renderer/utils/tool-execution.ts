...
const TOOL_INTENT_PATTERN =
  /(read|write|edit|create|delete|rename|move|run|execute|test|debug|fix|open|inspect|review|search|find|list|show|build|install|deploy|model|profile|config|setting|settings|skill|skills|tool|tools|web|api|switch|change|pro|flash|deepseek|\u542f\u52a8|\u8fd0\u884c|\u6267\u884c|\u6d4b\u8bd5|\u8c03\u8bd5|\u4fee\u590d|\u4fee\u6539|\u7f16\u8f91|\u521b\u5efa|\u65b0\u5efa|\u5220\u9664|\u91cd\u547d\u540d|\u79fb\u52a8|\u6253\u5f00|\u67e5\u770b|\u8bfb\u53d6|\u641c\u7d22|\u67e5\u627e|\u5217\u51fa|\u5ba1\u67e5|\u6784\u5efa|\u5b89\u88c5|\u90e8\u7f72|\u6587\u4ef6|\u76ee\u5f55|\u6587\u4ef6\u5939|\u4ee3\u7801|\u9879\u76ee|\u62a5\u9519|\u9519\u8bef|bug|\u7ec8\u7aef|\u547d\u4ee4|\u5de5\u5177|\u6a21\u578b|\u5e95\u5c42|\u914d\u7f6e|\u8bbe\u7f6e|\u5207\u6362|\u66f4\u6362|\u6539\u6210|\u6539|goal|mcp|看看|看一下|查一下|查查|检查|审查|分析|帮我看|帮我查|修一下|改一下|更新一下|继续|接着|往下|下一步|继续执行|继续任务|接着做)/i;

/**
 * Decide whether assistant tool calls should be honored for this user turn.
 * Keep obvious conversation prompts tool-free, but allow any request with
 * task intent or longer task-like text.
 */
 ...
 * Supports all built-in tool definitions and allows no-arg tools like listTools.
 */
export function parseXmlToolCalls(text: string): ToolCall[] {
  const toolCalls: ToolCall[] = [];
  const toolDefinitionsByName = new Map(TOOLS_DEFINITION.map((tool) => [tool.function.name, tool]));
  const toolNames = TOOLS_DEFINITION.map((tool) => tool.function.name);
  ...
  for (const toolName of toolNames) {
    ...
    while (true) {
      ...
      const parameters = toolDef?.function?.parameters as { required?: string[] } | undefined;
      const requiredCount = Array.isArray(parameters?.required) ? parameters.required.length : 0;
