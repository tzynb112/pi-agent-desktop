...
const FileViewerModal: React.FC<FileViewerModalProps> = ({ isOpen, filePath, content, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      // slight delay so the browser picks up the start state before transitioning
  ...
  // Keep panel mounted while animating out
  ...
  return (
    <>
      {/* Dim overlay (subtle, not blocking) */}
      <div
        onClick={onClose}
        style={{
          position: 'fixed',
      ...
      <div
        style={{
          ...
          transform: visible ? 'translateX(0)' : `translateX(${panelWidth}px)`,
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, marginLeft: 8 }}>
            ...
            <button
              ...
              onMouseEnter={(e) => {
                if (!wrapLines) e.currentTarget.style.color = 'var(--text-primary)';
              }}
              onMouseLeave={(e) => {
                if (!wrapLines) e.currentTarget.style.color = 'var(--text-muted)';
              }}
            >
